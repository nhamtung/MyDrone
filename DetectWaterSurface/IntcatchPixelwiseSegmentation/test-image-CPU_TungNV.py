...
for i in range(1):
    ...
    for models_name in model_path_name:
       # sess = tf.Session(config=config)
        ...
        print("TungNV_path_models: ", path_models+models_name)
        model = model_class.load_model(path_models+models_name)
        print("TungNV_model: ", model)

        dir_output = Utils.create_folder(os.path.join(folder_output, models_name))
        print ("TungNV_dir_output: ", dir_output)
        
        Utils.test_from_folder(folderName, dir_output, model, 160, 160)
