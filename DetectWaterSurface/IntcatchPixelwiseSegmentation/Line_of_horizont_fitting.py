...
class Line_of_horizont_fitting:
    ...
    def horizont_line_from_binary_image(self, image, average_n_frame=1, rho = 2, theta = np.pi/180, threshold = 20, min_line_length = 20, ax_line_gap = 5):
    ...
    def horizont_line_pipeline(self, image, model, img_w, img_h, average_n_frame, kernel_median_blur=50, predict_treshold=0.5, rho = 2, theta = np.pi/180, threshold = 20, min_line_length = 20, max_line_gap = 5):
        or_image=image
    ...
    def horizont_line_pipeline_TungNV(self, image, model, img_w, img_h, average_n_frame, kernel_median_blur=50, predict_treshold=0.5, rho = 2, theta = np.pi/180, threshold = 20, min_line_length = 20, max_line_gap = 5):
        or_image = image
        #print("TungNV_or_image: ", or_image)
        or_height, or_width, or_depth = or_image.shape
        #print("TungNV_or_image.shape: ", or_image.shape)
        image = self.inputNormalized(or_image,img_w,img_h)
        #print("TungNV_image: ", image)
        ...
        lines = self.hough_lines(output, rho, theta, threshold, min_line_length, max_line_gap)
        line_arr = np.squeeze(lines)
        #print("TungNV_line_arr: ", line_arr)

        if line_arr.size <= 4 :
            print("TungNV_line_arr.size: ", line_arr.size)
            return None, None
        else:
            points = self.Collect_points(line_arr)
            if(len(points)<2):
            ...
            if(len(points)>2):
                fit_line = cv2.fitLine(np.float32(points), cv2.DIST_HUBER, 1, 0.001, 0.001)
            return fit_line, predict #self.resize_image(predict_segmentation, or_width, or_height)
